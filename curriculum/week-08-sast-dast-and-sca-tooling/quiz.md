...
**B** — SAST never executes the program and reads only source; DAST sends real HTTP traffic to a live, running target and never looks at source at all.
...
**A** — a dangerous taint flow requires an untrusted source, a dangerous sink, and no recognized sanitizer breaking the path between them; missing any one of the three means the engine won't (and shouldn't) flag it as that specific flow.
...
**B** — the code reading a resource by ID is often syntactically ordinary; the vulnerability is the *absence* of an authorization check that was never written, and static pattern-matching has no dangerous shape to match against something that isn't there.
...
**A** — `pattern-not` explicitly excludes the safe shape (a parameterized call with a second, bound-parameters argument) so the rule only fires on the dangerous concatenation pattern, not on code that's already using the fix.
...
**B** — scanner severity reflects the rule author's general judgment about a pattern; your risk score reflects likelihood and impact specific to your actual app and context, which can genuinely diverge from the tool's generic rating.
...
**B** — the passive scan analyzes traffic already generated by the spider, with zero additional requests sent; only the active scan sends new, crafted payloads.
...
**B** — most of a real app's attack surface (checkout, profile, order history) sits behind login; an anonymous crawler structurally cannot reach any of it, so authenticated scans discover far more.
...
**B** — Risk (how bad if real) and Confidence (how sure the tool is this instance is real) are independent axes; a High-risk/Low-confidence alert deserves manual verification precisely because it might be a serious finding the tool is only partially sure about.
...
**B** — the header exists to stop browsers from dangerously rendering HTML in a risky context; a pure JSON API endpoint never framed by a browser has a real but low-priority version of this finding, and understanding why matters more than the raw "missing header" fact alone.
...
**C** — SCA answers whether specific installed library versions have a publicly disclosed, known vulnerability — a lookup against vulnerability databases, not a code-shape or live-traffic question.
...
**B** — a real CVE match against an installed library version can exist alongside the vulnerable function never actually being called in your app's code path, meaning the practical risk may be much lower than the CVE's raw severity implies.
...
**C** — reconciliation is a deliberate, evidence-based process: merge findings with a genuinely shared root cause, and explicitly document why distinct-but-similar-looking findings are kept separate; neither ignoring disagreement nor blindly merging by keyword is correct.
...
**B** — a false positive requires a specific written justification tied to actual evidence (e.g., confirmed sanitization) — a vague dismissal fails this course's triage discipline.
...
**B** — `severity_raw` preserves the original tool's rating as an audit trail; `severity_normal` enables consistent cross-tool ranking; both serve distinct, legitimate purposes and neither should be discarded.
